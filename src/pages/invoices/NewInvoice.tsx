...
const NewInvoice: React.FC<NewInvoiceProps> = ({ initialData }) => {
  ...
  const form = useForm<z.infer<typeof invoiceFormSchema>>({
    ...
    defaultValues: {
      ...
      dueDate: initialData ? new Date(initialData.dueDate) : new Date(new Date().setDate(new Date().getDate() + 14)),
      sellDate: initialData ? new Date(initialData.sellDate) : new Date(),
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Set default business profile only if we're not in edit mode and no profile is selected
        if (!isEditing && !form.getValues("businessProfileId") && defaultProfile) {
          form.setValue("businessProfileId", defaultProfile.id);
        }
        
        // Initialize items if in edit mode
  ...
  const onSubmit = async (values: z.infer<typeof invoiceFormSchema>) => {
    try {
      ...
      // Ensure all required fields are present for the invoice
      const invoice: Invoice = {
        id: initialData?.id || "", // This will be generated by the database if empty
        number: values.number,
        type: values.type,
        issueDate: values.issueDate.toISOString(),
        dueDate: values.dueDate.toISOString(),
        sellDate: values.sellDate.toISOString(),
        ...
        ksef: initialData?.ksef || { status: 'none' }
      };

      await saveInvoice(invoice);
      toast.success(isEditing ? "Faktura została zaktualizowana" : "Faktura została utworzona");
      navigate("/invoices");
    } catch (error) {
      console.error("Error saving invoice:", error);
      toast.error(isEditing ? "Wystąpił błąd podczas aktualizacji faktury" : "Wystąpił błąd podczas tworzenia faktury");
    }
  };
  ...
  return (
    <div className="pb-20">
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        <h1 className="text-2xl font-bold">{isEditing ? "Edytuj fakturę" : "Nowa Faktura"}</h1>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-background border-t p-4 flex justify-end gap-2 md:static md:border-0 md:p-0 md:bg-transparent">
            ...
            <Button type="submit">{isEditing ? "Zaktualizuj fakturę" : "Utwórz fakturę"}</Button>
          </div>
        </form>
      </Form>
