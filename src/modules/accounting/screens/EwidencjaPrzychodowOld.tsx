...
export default function EwidencjaPrzychodow() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'closed' | 'overdue'>('all');
  const [expandedPeriods, setExpandedPeriods] = useState<Set<string>>(new Set([`${new Date().getFullYear()}-${new Date().getMonth() + 1}`]));
