...
interface WizardData {
  ...
  // Step 3.5: Decisions (Organizational Mandates)
  selectedTemplateIds: string[]; // IDs of templates to create decisions from
...
const STEPS = [
  ...
  { id: 4, title: 'Zgody organizacyjne', icon: Shield },
  { id: 5, title: 'Potrzeby dokumentowe', icon: FileText },
  { id: 6, title: 'Usługi', icon: Briefcase },
  { id: 7, title: 'Konfiguracja', icon: FolderOpen },
];
...
export const SpoolkaWizard: React.FC<SpoolkaWizardProps> = ({ onComplete, onCancel, initialCompanyType }) => {
  ...
  const [wizardData, setWizardData] = useState<WizardData>({
    ...
    selectedTemplateIds: [], // Will be populated with foundational template IDs
    customDecisions: [],
      ...
      if (savedProfile.id) {
        ...
        if (wizardData.selectedTemplateIds.length > 0 || wizardData.customDecisions.length > 0) {
          ...
          for (const templateId of wizardData.selectedTemplateIds) {
            try {
              await createDecisionFromTemplate(templateId, savedProfile.id);
            } catch (error) {
          ...
          // Fallback: use default RPC function if no templates selected
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 4:
        return <Step4Decisions data={wizardData} updateData={updateData} />;
      case 5:
        return <Step5DocumentStorageNeeds data={wizardData} updateData={updateData} />;
      case 6:
        return <Step6Services data={wizardData} updateData={updateData} />;
      case 7:
        return <Step7FolderSetup data={wizardData} updateData={updateData} />;
      default:
        return null;
    }
  };
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 4:
        return wizardData.selectedTemplateIds.length > 0 || wizardData.customDecisions.length > 0;
      case 5:
...
// Step 4: Decisions (Organizational Mandates)
const Step4Decisions: React.FC<{
  data: WizardData;
  updateData: (field: keyof WizardData, value: any) => void;
}> = ({ data, updateData }) => {
  const [newDecisionTitle, setNewDecisionTitle] = useState('');
  const [newDecisionDescription, setNewDecisionDescription] = useState('');
  const [newDecisionType, setNewDecisionType] = useState<'strategic_shareholders' | 'operational_board'>('operational_board');
  const [newDecisionScope, setNewDecisionScope] = useState('');
  const [showAddForm, setShowAddForm] = useState(false);

  // Fetch templates from database
  const { data: templates = [], isLoading } = useQuery({
    queryKey: ['decision-templates', 'sp_zoo', true],
    queryFn: () => getRecommendedTemplates('sp_zoo', true),
  });

  // Auto-select foundational templates on first load
  React.useEffect(() => {
    if (templates.length > 0 && data.selectedTemplateIds.length === 0) {
      const foundationalIds = templates.filter(t => t.is_foundational).map(t => t.id);
      updateData('selectedTemplateIds', foundationalIds);
    }
  }, [templates, data.selectedTemplateIds.length]);

  const toggleTemplate = (templateId: string) => {
    const updated = data.selectedTemplateIds.includes(templateId)
      ? data.selectedTemplateIds.filter(d => d !== templateId)
      : [...data.selectedTemplateIds, templateId];
    updateData('selectedTemplateIds', updated);
  };

  const addCustomDecision = () => {
    if (!newDecisionTitle.trim()) {
      toast.error('Podaj tytuł decyzji');
      return;
    }
    ...
    const newDecision = {
      ...
      category: 'other',
      scope_description: newDecisionScope,
    };

    updateData('customDecisions', [...data.customDecisions, newDecision]);
    setNewDecisionTitle('');
    setNewDecisionDescription('');
    setNewDecisionScope('');
    setShowAddForm(false);
    toast.success('Dodano niestandardową decyzję');
  };

  const removeCustomDecision = (index: number) => {
    const updated = data.customDecisions.filter((_, i) => i !== index);
    updateData('customDecisions', updated);
  };

  const strategicTemplates = templates.filter(t => t.decision_type === 'strategic_shareholders');
  const operationalTemplates = templates.filter(t => t.decision_type === 'operational_board');

  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-2">Zgody organizacyjne (wymagane prawnie)</h3>
        <p className="text-sm text-muted-foreground mb-6">
          Wybierz domyślne decyzje, które będą utworzone dla Twojej spółki. Każda operacja w systemie wymaga powiązania z odpowiednią zgodą organizacyjną.
        </p>
      </div>

      {isLoading ? (
        <div className="text-center py-8 text-muted-foreground">
          Ładowanie szablonów decyzji...
        </div>
      ) : (
        <>
          {/* Strategic Decisions */}
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium">Uchwały wspólników (strategiczne)</h4>
              <Badge variant="secondary" className="text-xs">{strategicTemplates.filter(t => data.selectedTemplateIds.includes(t.id)).length}/{strategicTemplates.length}</Badge>
            </div>
            {strategicTemplates.map((template) => (
              <Card key={template.id} className={data.selectedTemplateIds.includes(template.id) ? 'border-purple-300' : ''}>
                <CardContent className="pt-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-medium">Uchwały zarządu (operacyjne)</h4>
              <Badge variant="secondary" className="text-xs">{operationalTemplates.filter(t => data.selectedTemplateIds.includes(t.id)).length}/{operationalTemplates.length}</Badge>
            </div>
            {operationalTemplates.map((template) => (
              <Card key={template.id} className={data.selectedTemplateIds.includes(template.id) ? 'border-blue-300' : ''}>
                <CardContent className="pt-4">
                  <div className="flex items-start space-x-3">
                    <Checkbox
                      id={template.id}
      ...
      {/* Custom Decisions */}
      {data.customDecisions.length > 0 && (
        <div className="space-y-3 pt-4 border-t">
          <h4 className="font-medium">Niestandardowe decyzje</h4>
          {data.customDecisions.map((decision, index) => (
              ...
              <CardContent className="pt-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-xs">
                        {decision.decision_type === 'strategic_shareholders' ? 'Wspólnicy' : 'Zarząd'}
                      </Badge>
                    </div>
                  ...
                  <Button
                    ...
                    onClick={() => removeCustomDecision(index)}
                  >
      ...
      {!showAddForm ? (
        <Button onClick={() => setShowAddForm(true)} variant="outline" className="w-full">
          ...
          Dodaj niestandardową decyzję
        </Button>
      ) : (
        <Card className="border-green-300">
          <CardContent className="pt-4 space-y-4">
            <div className="space-y-2">
              <Label htmlFor="new-decision-title">Tytuł decyzji *</Label>
              <Input
                id="new-decision-title"
                placeholder="np. Zgoda na najem nieruchomości"
                value={newDecisionTitle}
                onChange={(e) => setNewDecisionTitle(e.target.value)}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="new-decision-type">Typ decyzji</Label>
              <Select
                value={newDecisionType}
                onValueChange={(value: 'strategic_shareholders' | 'operational_board') => setNewDecisionType(value)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="strategic_shareholders">Uchwała wspólników</SelectItem>
                  <SelectItem value="operational_board">Uchwała zarządu</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="new-decision-description">Opis (opcjonalnie)</Label>
              <Textarea
                id="new-decision-description"
                placeholder="Krótki opis decyzji..."
            ...
            <div className="space-y-2">
              <Label htmlFor="new-decision-scope">Zakres (opcjonalnie)</Label>
              <Textarea
                id="new-decision-scope"
                placeholder="Określ zakres uprawnień..."
                value={newDecisionScope}
                onChange={(e) => setNewDecisionScope(e.target.value)}
                rows={2}
              />
            </div>
            <div className="flex gap-2">
              <Button onClick={addCustomDecision} className="flex-1">
                Dodaj
              </Button>
      ...
      <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-blue-900 dark:text-blue-100">Dlaczego to jest ważne?</p>
            <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
              Każda operacja w spółce (faktury, umowy, wydatki) musi być powiązana z odpowiednią zgodą organizacyjną. To wymóg prawny zapewniający audyt i zgodność z Kodeksem Spółek Handlowych.
            </p>
          </div>
        </div>
      </div>
...
// Step 5: Document Storage Needs
const Step5DocumentStorageNeeds: React.FC<{
  data: WizardData;
  updateData: (field: keyof WizardData, value: any) => void;
}> = ({ data, updateData }) => {
...
// Step 7: Folder Setup & Document Recommendations
const Step7FolderSetup: React.FC<{
  data: WizardData;
  updateData: (field: keyof WizardData, value: any) => void;
}> = ({ data, updateData }) => {
