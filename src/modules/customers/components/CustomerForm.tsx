...
const formSchema = z.object({
  ...
  taxId: z.string().optional().refine((val) => !val || val.length === 10, {
...
const CustomerForm = ({
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      business_profile_id: initialData?.business_profile_id || effectiveProfileId || "",
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      // Always use business_profile_id from form if not shared
      const resolvedBusinessProfileId = values.is_shared
        ? undefined
        : (values.business_profile_id || effectiveProfileId);
      
      console.log('CustomerForm onSubmit - Saving customer with:', {
        business_profile_id: resolvedBusinessProfileId,
        is_shared: values.is_shared,
        form_business_profile_id: values.business_profile_id,
        effectiveProfileId
      });
