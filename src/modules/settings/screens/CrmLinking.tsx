...
export default function CrmLinking() {
  ...
  const urlToken = searchParams.get('token') || 
                  ...
                  window.location.pathname.split('/settings/token/')[1] ||
  ...
  const fetchLinkRequest = async () => {
    try {
      // Always use Supabase edge functions for CRM
      const crmBaseUrl = 'https://vsbgcalxbexigazzaftm.supabase.co/functions/v1';

      // Call CRM edge function to validate the link request
      const response = await fetch(`${crmBaseUrl}/validate-crm-link`, {
        method: 'POST',
        headers: {
  ...
  const confirmLink = async () => {
    ...
    try {
      ...
      const response = await fetch(`${crmBaseUrl}/confirm-crm-link`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
