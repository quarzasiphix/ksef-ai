...
export const DiscussionPanel: React.FC<DiscussionPanelProps> = ({ 
  ...
  const { data: thread, isLoading: isLoadingThread } = useQuery({
    ...
    queryFn: async () => {
      ...
      return getOrCreateThreadForInvoice(invoiceId, selectedProfileId, `Dyskusja - Faktura ${invoiceNumber}`);
    },
  ...
  // 3. Subscription to new messages
  useEffect(() => {
    if (!thread?.id) return;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const newMessage = await postMessage(
        ...
        {
          messageType: 'text',
          isInternal: false // Default to public/shared messages for now
        }
      );
          ...
          <div className="space-y-4">
            {isLoadingMessages ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                <p>Brak wiadomości.</p>
                <p className="text-sm">Rozpocznij dyskusję z kontrahentem.</p>
              </div>
            ) : (
              messages.map((msg) => {
